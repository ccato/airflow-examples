...
from .aws_glue_job_hook import AwsGlueJobHook
...
class AWSGlueJobOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        self.script_args['--date'] = context.get('execution_date').strftime('%Y-%m-%d')

        self.log.info("Initializing AWS Glue Job: {}".format(self.job_name))
        glue_job_run = glue_job.initialize_job(self.script_args)

        error_message = 'Error message: {}'.format(glue_job_run['ErrorMessage']) if glue_job_run['ErrorMessage'] else ''
