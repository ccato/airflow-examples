# -*- coding: utf-8 -*-
#
# This is copied from https://github.com/apache/airflow/pull/4068/files 2019-04-23 // Frans Krojegård
#
...
class AwsGlueJobHook(AwsHook):
    ...
    def initialize_job(self, script_args=None):
        """
    ...
    def job_completion(self, job_name=None, run_id=None):
        ...
        while True:
            ...
            error_message = job_status['JobRun']['ErrorMessage'] if 'ErrorMessage' in job_status['JobRun'] else None

            if job_run_state in {'FAILED', 'STOPPED', 'SUCCEEDED'}:
                self.log.info("Exiting Job {} Run State: {}"
                              .format(run_id, job_run_state))
                return {'JobRunState': job_run_state, 'JobRunId': run_id, 'ErrorMessage': error_message}
            else:
